...
class KnowledgeBaseManager:
    ...
    def __init__(
        ...
        self.documents_dir = documents_dir or Path(
            DefaultPaths.KNOWLEDGE_BASE_DOCUMENTS_DIR.value
        )
        self.index_dir = index_dir or Path(
            DefaultPaths.KNOWLEDGE_BASE_INDICES_DIR.value
        )
            ...
            with open(json_file, "r") as f:
                ...
                for doc_data in data:
                    ...
                    if "last_updated" in doc_data and doc_data["last_updated"]:
                        doc_data["last_updated"] = datetime.fromisoformat(
    ...
    def _create_llama_documents(
        self, kb_docs: List[KnowledgeDocument]
    ) -> List[Document]:
        ...
        for kb_doc in kb_docs:
            ...
            if kb_doc.examples:
                text += "\n\nExamples:\n" + "\n".join(
                    f"- {ex}" for ex in kb_doc.examples
                )
    ...
    def build_index(self, domain: KnowledgeDomain) -> VectorStoreIndex:
        ...
        logger.info(
            f"Building index '{index_name}' with {len(filtered_docs)} documents"
        )
        ...
        index = VectorStoreIndex.from_documents(
            llama_docs, embed_model=self.embed_model, show_progress=True
        )
