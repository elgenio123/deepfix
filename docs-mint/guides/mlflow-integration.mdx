...
description: "Integrate DeepFix with MLflow for experiment tracking and artifact storage."
---
...
with mlflow.start_run(run_name="custom-run"):
    ...
    # Example of a custom severity metric
...
image_client = DeepFixClient(api_url="http://localhost:8844", mlflow_config=image_config)
tabular_client = DeepFixClient(api_url="http://localhost:8844", mlflow_config=tabular_config)
```
...
Use local file storage instead of a remote server:
...
1. **Store Only Necessary Artifacts**: Avoid logging everything.
2. **Use Relative Paths**: Use consistent artifact paths.
3. **Clean Up Old Runs**: Archive or delete old experiments as needed.
...
```python
# Solution: Verify artifact root is accessible and MLflow is configured correctly
mlflow_config = MLflowConfig(
    tracking_uri="http://localhost:5000",
...
- [Quickstart Guide](/getting-started/quickstart) – Basic DeepFix usage
- [Configuration Guide](/getting-started/configuration) – Configure DeepFix
