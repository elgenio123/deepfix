...
class KnowledgeBridge(Agent):
    ...
    def __init__(
        ...
        self.kb_manager = KnowledgeBaseManager(
            documents_dir=Path(documents_dir), embed_model=embed_model
        )
    ...
    def _generate_queries(self, request: AgentKnowledgeRequest) -> List[str]:
        ...
        LOGGER.debug(
            f"Generated {len(queries)} queries for {request.requesting_agent}."
        )
        return queries
    ...
    def _validate_evidence(
        ...
        retrieved_evidences: List[KnowledgeItem],
    ) -> List[EvidenceValidationResult]:
        """Validate evidence using DSPy"""

        with self._llm_context():
            out = self.evidence_validator(
                question=query, context=request, retrieved_evidences=retrieved_evidences
            )

        return out.result

    def _retrieve_evidence(
        self, query: str, domain: KnowledgeDomain, top_k: int = 5
    ) -> List[KnowledgeItem]:
    ...
    def _eval_one_query(
        self, query: str, request: AgentKnowledgeRequest
    ) -> Tuple[str, List[KnowledgeItem], List[EvidenceValidationResult]]:
        """Evaluate one query and return the results"""
        results = self._retrieve_evidence(
            query=query, domain=request.domain, top_k=request.max_results
        )
        validation_result = self._validate_evidence(
            query=query, request=request, retrieved_evidences=results
        )
    ...
    def _format_response(
        ...
        validation: List[EvidenceValidationResult],
    ) -> KnowledgeResponse:
    ...
    def forward(self, request: AgentKnowledgeRequest) -> List[KnowledgeResponse]:
        ...
        with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            for query, evidence, validation in executor.map(func, queries):
                r = self._format_response(
                    query=query, evidence=evidence, validation=validation
                )
                responses.append(r)
        end_time = time.time()
        LOGGER.info(
            f"{len(responses)} responses returned in {end_time - start_time:.2f}s"
        )
...
class KnowledgeBridgeReActAgent(KnowledgeBridge):
    ...
    def __init__(
        ...
        self.orchestrator = dspy.ReAct(
            KnowledgeBridgeReActSignature, tools=self.get_tools(), max_iters=max_iters
        )
    ...
    def get_tools(self) -> List[dspy.Tool]:
        return [
            ...
            dspy.Tool(
                name="top_k", desc="Get top k for a given request", func=self._get_top_k
            ),
        ]
    ...
    def forward(self, request: AgentKnowledgeRequest) -> List[KnowledgeResponse]:
        ...
        responses = []
        for query, evidence, validation in zip(
            out.queries, out.retrieved_evidences, out.evidence_validations
        ):
            r = self._format_response(
                query=query, evidence=evidence, validation=validation
            )
