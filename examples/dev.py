...
def run_deepchecks_vision():
    train, test = load_train_and_val_datasets(image_size=448, batch_size=8, num_workers=4, pin_memory=False)
    ...
    out = runner.run_suites(dataset_name=dataset_name, train_data=train_data.to_loader(), test_data=test_data.to_loader())
    print(out)
...
def run_deepchecks_tabular():
    ...
    runner.run_suites(dataset_name=dataset_name,train_data=train_data, test_data=test_data)
...
def run_dataset_ingestion():
    ...
    client.ingest_dataset(dataset_name=dataset_name, train_data=train_data, test_data=test_data, data_type="tabular",overwrite=True)
